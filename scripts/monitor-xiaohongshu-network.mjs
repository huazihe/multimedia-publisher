const port = Number(process.argv[2] || process.env.WEIBOT_XIAOHONGSHU_CDP_PORT || 57303)
...
function interesting(url = '') {
  return /xiaohongshu\.com|xhscdn\.com|xhscdn\.net/i.test(url)
    && /(publish|note|create|submit|draft|media|image|upload|creator|api|galaxy)/i.test(url)
}
...
ws.addEventListener('message', event => {
  ...
  if (payload.method === 'Network.requestWillBeSent') {
    ...
    events.push({
      ...
      url,
      postData: params.request?.postData?.slice?.(0, 1000),
    })
    return
  }
  ...
  if (payload.method === 'Network.responseReceived') {
    ...
    setTimeout(async () => {
      try {
        ...
        item.body = String(body.body || '').slice(0, 2000)
      } catch {}
    }, 250)
  }
})
...
try {
  ...
  console.error(`Monitoring Xiaohongshu network on ${port} for ${durationMs}ms...`)
