...
export class EastmoneyAdapter extends CodeAdapter {
  ...
  async checkAuth(): Promise<AuthResult> {
    try {
      ...
      const accountId = data.Result?.accountId || data.Result?.relatedUid;
      const accountName = data.Result?.accountName || data.Result?.nickName;

      if (data.Success === 1 && (accountId || accountName)) {
        return {
